...
function TrendBadge({ trend }: { trend: "improving" | "stable" | "needs_attention" }) {
  if (trend === "improving") return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400">
      <TrendingDown className="w-3 h-3" /> Improving
    </span>
  );
  if (trend === "needs_attention") return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400">
      <TrendingUp className="w-3 h-3" /> More errors
    </span>
  );
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg bg-[var(--surface)] border border-[var(--line-soft)] text-[var(--foreground)]/55">
  ...
  return (
    <div className="w-full p-4 md:p-8 space-y-5 md:space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--foreground)] tracking-tight">Your Progress</h1>
          <p className="text-sm text-[var(--foreground)]/55 mt-0.5 leading-relaxed">AI-powered analysis of your English learning journey.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          ...
          <div key={s.label} className="bg-[var(--surface-strong)] border-[1.5px] border-[var(--line)] rounded-[18px] p-4 sm:p-5 flex items-center gap-3 shadow-[0_2px_0_var(--line)]">
            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 ${s.bg}`}>
      ...
      <div className="bg-[var(--surface-strong)] border-[1.5px] border-[var(--line)] rounded-[22px] p-5 sm:p-7 shadow-[0_2px_0_var(--line)] space-y-5 sm:space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-primary-50 dark:bg-primary-900/30 flex items-center justify-center shrink-0 border border-primary-100 dark:border-primary-800">
              <Target className="w-5 h-5 text-primary-600" />
            </div>
            <div>
              <h2 className="font-bold text-[var(--foreground)] text-base">Level Progression</h2>
              <p className="text-xs text-[var(--foreground)]/40 mt-0.5 leading-relaxed">
        ...
        {learningCtx && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Lessons Done",  value: `${learningCtx.lessonsCompleted}`,     sub: "completed",                              icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
              ...
              { label: "Weak Area",     value: analytics?.topCategory ? (ERROR_LABEL[analytics.topCategory] ?? analytics.topCategory) : "—", sub: "most frequent", icon: TrendingDown, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-900/20" },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-3 px-3 py-3 bg-[var(--surface)] border border-[var(--line)] rounded-xl">
      ...
      {recommendations.length > 0 && (
        <div className="bg-[var(--surface-strong)] border-[1.5px] border-[var(--line)] rounded-[22px] p-5 sm:p-7 shadow-[0_2px_0_var(--line)] space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-[var(--foreground)] text-base">Recommended for You</h2>
              <p className="text-xs text-[var(--foreground)]/40 mt-0.5 leading-relaxed">Lessons selected based on your grammar error patterns</p>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {recommendations.slice(0, 4).map((r) => (
              <Link
                key={r.id}
                href={`/lessons/${r.id}`}
                className="flex flex-col gap-3 p-4 bg-[var(--surface)] border border-[var(--line)] rounded-[18px] hover:border-primary-200 dark:hover:border-primary-700 hover:-translate-y-0.5 hover:bg-[var(--surface-strong)] hover:shadow-md shadow-[0_2px_0_var(--line)] active:translate-y-[2px] active:shadow-none transition-all duration-150 group"
              >
                <div className="w-9 h-9 rounded-lg bg-primary-50 dark:bg-primary-900/30 flex items-center justify-center shrink-0 border border-primary-100 dark:border-primary-800">
                  <BookOpen className="w-4 h-4 text-primary-500" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-[var(--foreground)] group-hover:text-primary-700 dark:group-hover:text-primary-300 leading-snug">{r.title}</p>
                  <p className="text-[10px] text-[var(--foreground)]/40 mt-1.5 leading-relaxed">{r.reason}</p>
                </div>
      ...
      <div className="bg-[var(--surface-strong)] border-[1.5px] border-[var(--line)] rounded-[22px] p-5 sm:p-7 shadow-[0_2px_0_var(--line)] space-y-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[var(--surface)] flex items-center justify-center shrink-0 border border-[var(--line)]">
            <History className="w-5 h-5 text-[var(--foreground)]/55" />
          </div>
          <div>
            <h2 className="font-bold text-[var(--foreground)] text-base">Recently Completed</h2>
            <p className="text-xs text-[var(--foreground)]/40 mt-0.5 leading-relaxed">Your latest lessons and practice sessions</p>
          </div>
        </div>
        ...
        {recent.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
            {recent.slice(0, 9).map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-3.5 bg-[var(--surface)] border border-[var(--line)] rounded-xl">
                ...
                <span className="text-xs font-semibold text-[var(--foreground)]/80 truncate flex-1 min-w-0 leading-snug">
          ...
          <div className="flex flex-col items-center gap-3 py-12 text-[var(--foreground)]/35">
            <BookOpen className="w-10 h-10" />
            <p className="text-xs text-[var(--foreground)]/40 leading-relaxed">Complete lessons to see your history here</p>
          </div>
        )}
      </div>
